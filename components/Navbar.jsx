  ...
  return (
    <header className=" h-20 fixed w-full z-20 bg-white shadow-2xl">
      <div className=" hidden lg:block h-full w-full relative">
        ...
        <nav className=" hidden h-1/2 lg:w-1/3 xl:w-1/2 absolute top-1/2 left-1/2 lg:flex flex-row gap-5 -translate-x-2/3 -translate-y-1/2">
          {links.map((value, i) => (
            <Link
              href={`/${value.link}`}
              key={i}
              className={
                " w-1/4 h-full grid place-items-center bg-[#1B4896] rounded text-white translate-y-0 hover:-translate-y-1/3 transition duration-500 ease-out"
              }
            >
      ...
      <div className=" lg:hidden flex flex-row items-center justify-around w-full h-full">
        <Link href={"/"}>
          <Image
            alt="logo"
            src={"/images/logo.png"}
